...
fn main() {
    ...
    let enumeration_imports = quote! {
        use serde::{Deserialize, Serialize};
        use std::{borrow::Cow, fmt};
    };
    let enumeration_postdefs = quote! {
    ...
    let enumerations = iter::once(enumeration_imports)
        .chain(enumerations)
        .chain(iter::once(enumeration_postdefs));

    let structure_imports = quote! {
    ...
    let type_aliases = iter::once(type_alias_imports).chain(type_aliases);

    let request_imports = quote! {
        #[allow(clippy::wildcard_imports)]
        use super::*;
    };
    let requests = iter::once(request_imports.clone()).chain(requests);
    let notifications = iter::once(request_imports).chain(notifications);

    let enum_or_imports = quote! {
        use serde::{Deserialize, Serialize};
        use std::borrow::Cow;
    ...
    let enum_ors = iter::once(enum_or_imports).chain(enum_ors);

    fn format(tokens: impl Iterator<Item = TokenStream>) -> String {
        let formatted_items: Vec<String> = tokens
            .map(|request| {
    ...
    fs::write("src/generated/common.rs", format(common)).unwrap();
    fs::write("src/generated/enumerations.rs", format(enumerations)).unwrap();
    fs::write("src/generated/structures.rs", format(structures)).unwrap();
    fs::write("src/generated/type_aliases.rs", format(type_aliases)).unwrap();
    fs::write("src/generated/requests.rs", format(requests)).unwrap();
    fs::write("src/generated/notifications.rs", format(notifications)).unwrap();
    fs::write("src/generated/enum_ors.rs", format(enum_ors)).unwrap();
